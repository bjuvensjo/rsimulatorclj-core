...
(defn
  ^{:doc  "Normalize the request appropriate for the extension"
    :test (fn []
  ...
  normalize [request extension]
  (case extension
    "xml" (.substring (xml/emit-str (xml/parse-str request)) 38) ;; Remove xml prolog and format
    request))

(defn
  ^{:doc  "Tries to find a match for the specified request in the files <rootPath>/<rootRelativePath>/*<extension>."
    :test (fn []
  ...
  findMatch [rootPath rootRelativePath extension request]
...
(defn
  ^{:doc  "Get the response that corresponds to the matchingRequestFile replaced with matches."
    :test (fn []
  ...
  getResponse [matchingRequestFile matches]
...
(defn
  ^{:doc  "Get config that corresponds to the matchingRequestFile"
    :test (fn []
...
(defn
  ^{:doc  "Returns a response of the specified request if such can be found recursively in <rootPath>/<rootRelativePath> in a request/response pair with the specified extension."
    :test (fn []
